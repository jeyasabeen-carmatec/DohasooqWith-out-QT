...
- (void)viewDidLoad {
    ...
    self.screenName = @"Product Filter screen";
    ...
    [self.collection_produtcs registerNib:[UINib nibWithNibName:@"Filter_cell_Arabic" bundle:nil]  forCellWithReuseIdentifier:@"cellA"];
    
    [self.collection_produtcs registerNib:[UINib nibWithNibName:@"filter_cell" bundle:nil]  forCellWithReuseIdentifier:@"cell"];
...
//    return cell;
    if([[[NSUserDefaults standardUserDefaults] valueForKey:@"story_board_language"] isEqualToString:@"Arabic"])
    {
        ...
        Filter_cell_Arabic *cell = (Filter_cell_Arabic *)[collectionView dequeueReusableCellWithReuseIdentifier:@"cellA" forIndexPath:indexPath];
        [cell.contentView setTransform:CGAffineTransformMakeScale(-1, 1)];
        cell.LBL_name.text = [product_arr objectAtIndex:indexPath.row];
        cell.BTN_check.tag = indexPath.row;
        [cell.BTN_check addTarget:self action:@selector(check_action:) forControlEvents:UIControlEventTouchUpInside];
