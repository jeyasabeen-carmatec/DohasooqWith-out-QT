...
{
    UIView *shadowView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.bounds.size.width +radius, self.bounds.size.height)];
    ...
    for (NSString *direction in directionDict) {
        ...
        if ([kValidDirections containsObject:direction])
        {
            ...
            else if ([direction isEqualToString:@"bottom"])
            {
                ...
                shadow.frame = CGRectMake(0, self.bounds.size.height - radius, self.bounds.size.width +radius, radius);
