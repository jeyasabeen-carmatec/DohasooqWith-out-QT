...
- (void)animateViewsForTextEntry {
    if (self.text.length == 0) {
        [UIView animateWithDuration:0.35 delay:0 usingSpringWithDamping:0.8 initialSpringVelocity:1.0 options:UIViewAnimationOptionBeginFromCurrentState animations:^{
            ...
            NSString *text = [NSString stringWithFormat:@"%@%@",[arr objectAtIndex:0],[arr objectAtIndex:1]];
            
            
            
            if ([self.placeholderLabel respondsToSelector:@selector(setAttributedText:)]) {
                ...
                NSRange ename = [text rangeOfString:[arr objectAtIndex:1]];
    ...
    if (self.text.length == 0) {
        [UIView animateWithDuration:0.35 delay:0 usingSpringWithDamping:0.8 initialSpringVelocity:2.0 options:UIViewAnimationOptionBeginFromCurrentState animations:^{
            ...
            if ([self.placeholderLabel respondsToSelector:@selector(setAttributedText:)]) {
                ...
                {
                    [attributedText setAttributes:@{NSFontAttributeName:[UIFont fontWithName:@"Poppins-Regular" size:12.0],NSForegroundColorAttributeName:[UIColor colorWithRed:0.99 green:0.68 blue:0.16 alpha:1.0]}
                                            range:ename];
                }
                self.placeholderLabel.attributedText = attributedText;
            }
